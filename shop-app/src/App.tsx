...
function normalizedRoleName(name: string) {
  return name.replace(/[♛♕⚕✦◇◆•·⋅]/g, "").replace(/\s+/g, " ").trim().toLocaleUpperCase("tr-TR")
...
function prepareCatalog(items: Product[]): Product[] {
  ...
  for (const item of items) {
    ...
    // Discord role names can arrive with an icon prefix (or with the old
    // AYYILDIZI spelling). Treat both forms as the same shop product.
  ...
  return prepared.sort((a, b) => {
    const aIndex = a.category === "Roller" ? shopRoleOrder.indexOf(normalizedRoleName(a.name)) : -1;
    const bIndex = b.category === "Roller" ? shopRoleOrder.indexOf(normalizedRoleName(b.name)) : -1;
