...
export default function App() {
  ...
  const frArdaUserId = "1231243551053053982";
  const canSeeManagement = user.isOwner === true || user.id === frArdaUserId;
  ...
  const loadCatalog = async (token: string) => {
    ...
    const missingRoles = demoProducts.filter((item) =>
      ["elite", "vip"].includes(item.id) && !hasRole(item.id, item.name)
    );
  ...
  const buy = async () => {
    ...
    try {
      ...
      setToast("Sipariş oluşturuldu. Coin bakiyen güncellendi.");
       ...
       setToast(error instanceof Error && error.message === "insufficient_balance" ? "Yeterli coin bakiyen yok." : error instanceof Error && error.message === "out_of_stock" ? "Bu ürün stokta kalmadı." : "Sipariş oluşturulamadı.");
    }
  };
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="side-links">
        ...
        {canSeeManagement && <a className="side-link side-admin" href="/admin.html" target="_blank" rel="noreferrer"><i className="admin-lock" aria-hidden="true" /> <span>Yönetim</span></a>}
      </div>
